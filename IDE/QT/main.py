...
class VisuRadionIDE(QMainWindow):
    ...
    def uploadToController(self):
        try:
            ...
            subprocess.run(['cp', SAVE_PATH + '/blocks.cpp', PATH_RUDIRON])
            RadionControllerService.compile(PATH_RUDIRON)
...
class RadionControllerService:
    ...
    @ staticmethod
    def compile(file_path: str | os.PathLike = PATH_RUDIRON) -> str:
        print(f"Компиляция файла: {file_path}")
        subprocess.run(['ls', file_path])
